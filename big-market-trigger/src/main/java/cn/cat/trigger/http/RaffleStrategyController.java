...
import cn.cat.domain.activity.service.IRaffleActivityAccountQuotaService;
...
import cn.cat.trigger.api.IRaffleStrategyService;
...
public class RaffleStrategyController implements IRaffleStrategyService {
     ...
     * 请求参数 raw json
     *
     * @param request {"userId":"cat", "activityId":100601}
     * @return 奖品列表
     */
    ...
    public Response<List<RaffleAwardListResponseDTO>> queryRaffleAwardList(@RequestBody RaffleAwardListRequestDTO request) {
        try {
            ...
            Response<List<RaffleAwardListResponseDTO>> response = Response.<List<RaffleAwardListResponseDTO>>builder()
                    ...
                    .build();
            log.info("查询抽奖奖品列表配置完成 userId:{} activityId：{} response: {}", request.getUserId(), request.getActivityId(), JSON.toJSONString(response));            // 返回结果
            return response;
        } catch (Exception e) {

...
import cn.cat.domain.task.service.ITaskService;
            ...
            for (int dbIdx = 1; dbIdx <= dbCount; dbIdx++) {
                ...
                executor.execute(() -> {
                    try {
                        ...
                        log.info("定时任务，扫描MQ任务表，当前库：{}", finalDbIdx);
                        dbRouter.setTBKey(0);
                        List<TaskEntity> taskEntities = taskService.queryNoSendMessageTaskList();
                        if (taskEntities.isEmpty()) return;
                        // 发送MQ消息
                        for (TaskEntity taskEntity : taskEntities) {
                            // 开启线程发送，提高发送效率。配置的线程池策略为 CallerRunsPolicy，在 ThreadPoolConfig 配置中有4个策略，面试中容易对比提问。可以检索下相关资料。@小傅哥
                            executor.execute(() -> {
                                try {
